...
侧边栏和应用的图标目前只有**8个**，当应用超过**8个**的时候,会循环拿图，会产生部分侧边栏的 `icon` 和应用的图标出现重复的现象，
但不影响使用。如需增加图标和 `icon` 的数量，在 `/src/localConfig` 目录下的 `icon.js` 和 `logo.js` 文件修改。其中 `icon.js` 在数组中加入
